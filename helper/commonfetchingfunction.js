...
module.exports = class fetching
{
    ...
    static isUserBlockedFromLoginAfterMaxPasswordAttempts(id)
    {
        return new Promise((resolve, reject) =>
        {
            let selQuery = `SELECT * FROM login_incorrect_attempts l WHERE l.user_Id = '${id}' AND l.incorrect_count = '${constants.password_protection_policy_numbers.number_of_incorrect_password_attempt}' AND l.blocked_till >= '${time.nowd()}' `;
            con.query(selQuery, (err, result) =>
            {
                if(result.length != 0)
                {
